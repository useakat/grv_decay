...
c     First try system wide (for cluster if define)
            ...
            tempname='/cvmfs/cp3.uclouvain.be/madgraph/'//Tablefile
            open(IU,file=tempname,status='old',ERR=1)
            return
 1          tempname='/cvmfs/cp3.uclouvain.be/madgraph//Pdfdata/'//Tablefile
            open(IU,file=tempname,status='old',ERR=2)
            return
 2          tempname='/cvmfs/cp3.uclouvain.be/madgraph//lhapdf'//Tablefile
            open(IU,file=tempname,status='old',ERR=3)
            return            
 3          tempname='/cvmfs/cp3.uclouvain.be/madgraph//../lhapdf/pdfsets/'//Tablefile
            open(IU,file=tempname,status='old',ERR=4)
            return              
 4          tempname='/cvmfs/cp3.uclouvain.be/madgraph//../lhapdf/pdfsets/6.1/'//Tablefile
